...
for json_file in `find . -type f -name "*.json"`; do
  ...
  lwjgl_version=$(cat "$json_file" | jq -r '.libraries | .[] | .name | select(contains("org.lwjgl:lwjgl:"))' | sed 's/org.lwjgl:lwjgl://g' | sed 's/:.*//' | sort -g -r | head -1)
  ...
  jq '(.versions[] | select(.id == "'${json_file:2:-5}'") | .sha1) |= "'$sha1'" | (.versions[] | select(.id == "'${json_file:2:-5}'") | .url) |= "'https://github.com/theofficialgman/piston-meta-arm32/raw/main/version-json/$json_url'"' "$GIT_DIR/mc/game/version_manifest_v2_noncompact.json" | sponge "$GIT_DIR/mc/game/version_manifest_v2_noncompact.json"
